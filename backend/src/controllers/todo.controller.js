...
exports.createTodo = async (req, res) => {
  try {
    ...
    const todo = await SimpleTodo.create({
      title,
      description,
      visibility: req.user.role === "guest" ? "private" : visibility,
      owner: req.user._id,
      useSteps,
    });
...
exports.getTodos = async (req, res) => {
  try {
    ...
    if (role === "admin") query = {};
      ...
      query = { $or: [{ owner: _id }, { visibility: "public" }] };
    else query = { $or: [{ owner: _id }, { visibility: "public" }] };
    const todos = await SimpleTodo.find(query)
  ...
  try {
    ...
    if (req.params.stepId) {
      ...
      if (todo.steps.length === 0)
        todo.steps.push({ title: "Items", order: 0 });
      step = todo.steps[0];
    }
